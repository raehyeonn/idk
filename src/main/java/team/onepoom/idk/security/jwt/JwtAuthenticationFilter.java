...
public class  JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    private static final List<String> PUBLIC_PATHS = Arrays.asList("/api/auth/login", "/api/members/verify-email");
    private final JwtUtils jwtUtils;

    protected void doFilterInternal(HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse, FilterChain filterChain) throws IOException, ServletException {
        String accessToken = jwtUtils.extractTokenFromRequestHeader(httpServletRequest);
        ...
        if(authentication == null && refreshToken != null) {
            ...
            accessToken = jwtUtils.extractTokenFromResponseHeader(httpServletResponse);
            tokenRefreshed = true;
        }
    ...
    protected boolean shouldNotFilter(HttpServletRequest httpServletRequest) {
        ...
        || ("GET".equalsIgnoreCase(method) && path.equals("/api/questions"))
        || ("GET".equalsIgnoreCase(method) && path.matches("/api/questions/\\d+"))
        || ("GET".equalsIgnoreCase(method) && path.startsWith("/api/notices"));
