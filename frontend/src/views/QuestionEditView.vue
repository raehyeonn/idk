...
import {editQuestionAPI, getQuestionAPI} from "@/api";
...
const editQuestion = async function () {
    try {
        ...
        await editQuestionAPI(questionId.value, editQuestionRequest);
        await router.push(`/question/${questionId.value}`);
    } catch (error) {
        console.log(error);
    }
};
...
onMounted(async () => {
    questionId.value = useRoute().params.questionId;
    const response = await getQuestion(questionId.value);
...
<template>
    <div class="edit-wrap">
        ...
        <div class="edit-button">
            ...
            <button class="submit-button" @click="editQuestion">수정하기</button>
        </div>
    </div>
</template>
